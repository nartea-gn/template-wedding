...
describe('LoginForm', () => {
    it('requests an OTP from the authorized email', async () => {
        ...
        await user.type(screen.getByRole('textbox', {name: 'Correo electrónico'}), 'Admin@Example.com')
    ...
    it('authenticates the password variant without exposing the OTP controls', async () => {
        ...
        expect(onAuthenticate).toHaveBeenCalledWith('admin@example.com', 'private-password')
        expect(screen.queryByText('Código de seis dígitos')).not.toBeInTheDocument()
    })
